...
EkkoLightbox.prototype = {
	modal_shown: ->
		...
		if !@options.remote && !@options.type=='text'
	...
	getVimeoId: (str) ->
      	...
      	if match && match[2].length > 0 then match[2] else false
	...
	detectRemoteType: (src, type) ->
		...
		if type == 'image' || @isImage(src)
			...
			video_id = @getYoutubeId(src)
			@options.type = 'youtube'
			@showYoutubeVideo(video_id)

		else if type == 'vimeo'
			video_id = @getVimeoId(src)
			@options.type = 'vimeo'
			@showVimeoVideo(video_id)

		else if type == 'instagram'
			video_id = @getInstagramId(src)
	...
	showYoutubeVideo : (id) ->
		...
		@resize width
		@lightbox_body.html '<iframe width="'+width+'" height="'+height+'" src="http://www.youtube.com/embed/' + id + '?badge=0&autoplay=1&html5=1" frameborder="0" allowfullscreen></iframe>'
	...
	showVimeoVideo : (id) ->
		...
		height = width / aspectRatio
		@resize width
		@lightbox_body.html '<iframe width="'+width+'" height="'+height+'" src="http://player.vimeo.com/video/' + id + '?autoplay=1" frameborder="0" allowfullscreen></iframe>'
		if @modal_arrows #hide the arrows when showing video
			@modal_arrows.css 'display', 'none'
